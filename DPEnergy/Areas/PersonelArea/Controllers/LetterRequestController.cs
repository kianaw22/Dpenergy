...
namespace DPEnergy.Areas.PersonelArea.Controllers
{
    ...
    public class LetterRequestController : Controller
        {
            ...
            public LetterRequestController(IUnitOfWork uow, IMapper mapper, UserManager<A_UserManager> userManager, IUploadFiles upload, ApplicationDbContext dbcontext)
            {
            ...
            public IActionResult Index()
            {
                ...
                var model = _context.LetterRequestUW.Get();
                return View(model);
            }
            [HttpGet]
            public IActionResult AddLetterRequest()
            {
                FillCombo();
                return View();
            }
            ...
            {
                var LetterRequest = _dbcontext.P_LetterRequest.Where(e => e.PersonelCode == personelCode);
                if (LetterRequest.Any())
                {
                    var maxCounter = LetterRequest.Max(e => e.Counter);
                    return maxCounter + 1;
                }
                return 1;
            }
        ...
        public IActionResult DeleteAttachment(string path, string id)
            {
                ...
                if (id != null)
                {
                    var record = _dbcontext.P_LetterRequest.First(x => x.Id == id);
                    if (!string.IsNullOrEmpty(record.Attachment))
                    {
                        record.Attachment = null;
                        _context.LetterRequestUW.Update(record);
                        _context.save();
                    }
                }
            ...
            public IActionResult AddLetterRequest(P_LetterRequestViewModel model, string issuedatee, string expiredatee)
            {
                FillCombo();
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (expiredatee != null)
                    {
                        model.ExpirationDate = ConvertDateTime.ConvertShamsiToMiladi(expiredatee);
                    }

                    _context.LetterRequestUW.Create(_mapper.Map<P_LetterRequest>(model));
                    _context.save();
            ...
            [HttpGet]
            public IActionResult EditLetterRequest(String Id)
            {
                FillCombo();
                if (Id == null)
                {
                ...
                var comp = _context.LetterRequestUW.GetById(Id);
                var mapcomp = _mapper.Map<P_LetterRequestViewModel>(comp);
                return View(mapcomp);

            }
            [HttpPost]
            [ValidateAntiForgeryToken]
            public IActionResult EditLetterRequest(P_LetterRequestViewModel model, string issuedatee, string expiredatee)
            {
                FillCombo();
                ...
                var compmapper = _mapper.Map<P_LetterRequest>(model);
                    _context.LetterRequestUW.Update(compmapper);
                    _context.save();
                    return RedirectToAction("Index");
                }
                return View(model);
            ...
            [HttpGet]
            public IActionResult DeleteLetterRequest(string Id)
            {
                if (Id == null)
                {
                    return RedirectToAction("ErrorView", "Home");
                }
                var city = _context.LetterRequestUW.GetById(Id);
                if (city == null)
                {
                    return RedirectToAction("ErrorView", "Home");
                }
                return PartialView("_deleteLetterRequest", city);
            }
            [HttpPost]
            [ValidateAntiForgeryToken]
            public IActionResult DeleteLetterRequestPost(string Id)
            {
                if (Id == null)
                {
                    return RedirectToAction("ErrorView", "Home");
                }
                var model = _context.LetterRequestUW.GetById(Id);
                DeleteAttachment(model.Attachment, null);
                _context.LetterRequestUW.DeleteById(Id);
                _context.save();
                return RedirectToAction("Index");
